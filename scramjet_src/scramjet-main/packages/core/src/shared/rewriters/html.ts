...
export class IncrementalHtmlRewriter {
	...
	private flush() {
		...
		for (const node of (this.handler.root.childNodes || this.handler.root.children || [])) {
			...
			if (rewritten && rewritten.length > emittedLength) {
...
function rewriteHtmlInner(
	...
	function detectQuirks() {
		for (const child of (handler.root.childNodes || handler.root.children || [])) {
			if (
		...
		for (const child of (htmlRoot.childNodes || htmlRoot.children || [])) {
			if (
				child.type === ElementType.Directive ||
				child.type === ElementType.Comment ||
				child.type === ElementType.Text
...
function traverseParsedHtml(
	...
	if (children) {
		for (const childNode in children) {
			children[childNode] = traverseParsedHtml(
				children[childNode],
				context,
				meta
			);
